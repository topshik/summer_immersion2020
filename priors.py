...
def log_normal_diag(z, mean, log_var,  dim=None):
    log_normal = -0.5 * (z.shape[1] * math.log(2.0 * math.pi) + log_var.sum(dim=1) + (torch.pow(z - mean, 2) /
                         (torch.exp(log_var) + 1e-5)).sum(dim=1))
    print('log_normal', log_normal.shape)
    return log_normal
...
class SimpleGaussian(Prior):
    def __init__(self, batch_size, latent_size) -> None:
        super().__init__()
        # TODO make nn.Parameter
        self.batch_size = batch_size
        self.latent_size = latent_size

    def log_p_z(self, z) -> torch.Tensor:
        return - 0.5 * (self.latent_size * math.log(2 * math.pi) + z.pow(2)).sum(dim=1)
